...
library SECP256K1 {
  ...
  static function negatePoint(Point p) : Point {
    return {p.x, SECP256K1.modReduce(p.y * -1, 115792089237316195423570985008687907853269984665640564039457584007908834671663)};
  }
  static function doublePoint(Point p) : Point {
    int lambda = 3 * p.x * p.x * SECP256K1.modInverseBranchlessP(2 * p.y);
    int rx = SECP256K1.modReduce(lambda * lambda - 2 * p.x, 115792089237316195423570985008687907853269984665640564039457584007908834671663);
    int ry = SECP256K1.modReduce(lambda * (p.x - rx) - p.y, 115792089237316195423570985008687907853269984665640564039457584007908834671663);
    return {rx, ry};
  }
  static function addPoints(Point p, Point q) : Point {
    ...
    if(p.x == 0 && p.y == 0) {
        ...
        int lambda = SECP256K1.modReduce(N * SECP256K1.modInverseBranchlessP(D), 115792089237316195423570985008687907853269984665640564039457584007908834671663);
        int rx = SECP256K1.modReduce(lambda * lambda - p.x - q.x, 115792089237316195423570985008687907853269984665640564039457584007908834671663);
        int ry = SECP256K1.modReduce(lambda * (p.x - rx) - p.y, 115792089237316195423570985008687907853269984665640564039457584007908834671663);
        ret = {rx, ry};
      }
    return ret;
  ...
  static function point2PubKey(Point point) : PubKey {
    return PubKey(b'04' + SECP256K1.toBEUnsigned32(point.x) + SECP256K1.toBEUnsigned32(point.y));
  }
  ...
  static function verifySig(int hashInt, Signature sig, Point pubKey) : bool {
    require(sig.r >= 1 && sig.r < 115792089237316195423570985008687907852837564279074904382605163141518161494337 && sig.s >= 1 && sig.s < 115792089237316195423570985008687907852837564279074904382605163141518161494337);
    int sInv = SECP256K1.modInverseBranchlessN(sig.s);
    int u1 = SECP256K1.modReduce(hashInt * sInv, 115792089237316195423570985008687907852837564279074904382605163141518161494337);
    int u2 = SECP256K1.modReduce(sig.r * sInv, 115792089237316195423570985008687907852837564279074904382605163141518161494337);
    Point U1 = SECP256K1.mulGeneratorByScalar(u1);
    Point U2 = SECP256K1.mulByScalar(pubKey, u2);
    Point X = SECP256K1.addPoints(U1, U2);
    return sig.r == X.x;
  }
}